...
const VideoCard = ({ video: {title, thumbnail, video, creator:{username, avatar}} }) => {
    ...
    <View className='flex-col items-center px-4 mb-14'>
      ...
      {play ? (
        <Video source={{uri:video}} className='w-full h-72 rounded-[35px] mt-3 ' resizeMode={ResizeMode.CONTAIN} useNativeControls shouldPlay onPlaybackStatusUpdate={(status)=> { if(status.didJustFinish){setPlay(false);}}}/>
      ) : (
