...
const Create = () => {
  ...
  const openPicker= async (selectType) => {
    ...
    if(!result.canceled){
      if(selectType==='image')
      {
        // console.log('Selected thumbnail:', result.assets[0]);  // Debug log
  ...
  const submit = async () => {
    ...
    try {
      ...
      await createVideo({
        ...form, userId: user.$id, thumbnailUrl:form.thumbnail.uri
      });

      Alert.alert('Success', 'Post uploaded successfully');
      router.push('/home');
    } catch (error) {
      Alert.alert('Sab se bara Error', error.message);
    } finally {
      setForm({
        title:'',
        video:null,
  ...
  return (
    <SafeAreaView className='bg-primary h-full'>
      <ScrollView className='px-4 my-6'>
        ...
        <FormField title="Prompt" value={form.prompt} placeholder="The prompt for your video" handleChangeText={(e)=>setForm({...form, prompt:e})} otherStyles="mt-7" />
